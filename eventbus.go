...
func (p *Message) GetPortName() string {
	return p.Port.Name
}

func (p *Message) GetPort() *Port {
	return p.Port
}

func (p *Message) FromObject() string {
	return p.ObjectUUID
}
func (p *Message) FromObjectID() string {
	return p.ObjectID
}
...
func (eb *EventBus) Subscribe(topic string, handlerID string, handler func(e event.Event) error) {
	...
	listenerFunc := event.ListenerFunc(handler)
	...
	// Add the listener function to the registry with an identifier
...
func (eb *EventBus) Unsubscribe(topic string, handlerID string) {
	...
	listeners, ok := eb.handlerRegistry[topic]
	if !ok {
	...
	// If no more listeners for this topic, optionally remove the topic
	if len(listeners) == 0 {
		delete(eb.handlerRegistry, topic)
		eb.manager.RemoveEvent(topic) // Remove the entire event from the manager
	}
...
func (eb *EventBus) Publish(topic string, data *Message) {
	...
	eb.manager.Fire(topic, eventData)
}
